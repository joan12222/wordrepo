...
- **index.html**（约 1410 行）：完整 PWA 单词记忆工具，单文件实现全部功能
  - 生词本管理：首次打开选择导入 JSON 或新建，数据存 localStorage
  - PDF 解析：pdf.js 3.11.174 CDN，利用位置/字号信息识别标题(h2/h3)和正文段落，排版层次化渲染，双击英文单词标记/取消标记；自动清除乱码字符；多页间插入分隔线；自动检测试卷格式并对选项缩进
  ...
  - 单词复习：按熟练度多选筛选，每词 2 题（看单词选中文 + 看释义选单词），四选一，错误选项从全库随机抽取，答后显示解析
...
- 入口文件：`index.html`（1384 行）
- 关键函数行号（以当前文件为准）：
  - 生词本渲染：`renderVocab()` 第 592 行
  - 批量删除：`batchDelete()` 第 676 行，`doBatchDelete()` 第 687 行
  - 单词详情（含删除）：`showDetail()` 约第 676 行前
  - PDF 文本清洗：`cleanPDFText()` 第 748 行
  - PDF 解析：`loadPDF()` 第 757 行，`renderPDFText()` 第 822 行
  - 模式切换：`setGenMode()` 约第 841 行
  - 手动模式提示词生成：`genManualPrompt()` 约第 852 行
  - 手动模式 JSON 导入：`importManualJSON()` 约第 882 行
  - Claude API 调用：`claudeChunk()` 约第 1002 行
  - 复习题目生成：`buildQs()` 约第 1110 行
  - 数据导入/导出：`importJSON()` 约第 1262 行，`exportJSON()` 约第 1308 行
...
- **pdf.js Worker**：使用 CDN `pdf.worker.min.js` 同版本路径，避免跨域问题
